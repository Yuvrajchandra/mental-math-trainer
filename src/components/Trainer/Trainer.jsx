...
const Trainer = () => {
  ...
  useEffect(() => {
    const generateProblem = () => {
      ...
      let operand1 = Math.floor(Math.random() * (maxRange - minRange + 1) + minRange);
      let operand2;

      // Ensure division is perfect
      if (operator === 'division') {
        operand2 = Math.floor(Math.random() * (maxRange - minRange + 1) + minRange);
        operand2 = operand2 !== 0 ? operand2 : operand2 + 1; // Avoid division by zero
        operand1 *= operand2; // Ensure perfect division
      } else {
        operand2 = Math.floor(Math.random() * (maxRange - minRange + 1) + minRange);
      }
  ...
  const handleSubmit = () => {
    ...
    // if (problemsAnswered + 1 < problems.length) {
    //   setProblemsAnswered((prev) => prev + 1);
    // } else if (problemsAnswered + 1 === problems.length) {
    //   // Show the score if all problems are answered
    //   showScore();
    // }
  };
    ...
    <div>
      ...
      <div>
        ...
        {problemsAnswered < problems.length && (
          <>
            <input type="text" value={userAnswer} onChange={handleInputChange} onKeyPress={handleKeyPress} />
